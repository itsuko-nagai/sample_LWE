 ...
 * X^N == -1, so when a term's exponent i+j >= N it wraps around
 * with a sign flip. This is mathematically identical to what
...
static void poly_uniform_from_seed(const uint8_t seed[SEED_BYTES],
                                    uint8_t i, uint8_t j, poly *out)
{
    uint8_t buf[SEED_BYTES + 2]; /* seed is always exactly SEED_BYTES now,
                                     so this buffer is always exactly the
                                     right size -- no overflow possible,
                                     unlike the variable-length version */
...
static void decrypt_msg(const polyvec *s, const polyvec *u, const poly *v, uint8_t msg[32])
{
    poly su, mp;
    polyvec_dot(s, u, &su);
    poly_sub(v, &su, &mp);
...
int main(void)
{
    ...
    uint8_t msg[32];
    memset(msg, 0, 32);
    memcpy(msg, "HELLO-KYBER-MSG-DIAT!", 22); /* first 17 bytes carry a readable payload */
